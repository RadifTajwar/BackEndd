...
async function imagePromptGeneration(prpt) {
    ...
    const prewrittenPrompts = [
        ...
         one single string explain what the character doing . If the character is holding or doing something with his arms or legs or tail or head how they are doing it? give the result in a string named "prompt"`
    ];
...
exports.imageGenerate = catchAsyncError(async (req, res, next) => {
    ...
    const promptRegex1 = /prompt='(.*?)'/;
    const promptRegex2 = /'prompt'='([^']*)'/;
    const promptRegex3 = /\*\*Prompt\*\*: ([^\n]*)/;
    // Extracting the prompt value using match function
    const matchPrompt1 = reply.match(promptRegex1);
    const matchPrompt2 = reply.match(promptRegex2);
    const matchPrompt3 = reply.match(promptRegex3);
    
    var promptValue="";
    if(matchPrompt1){
        promptValue= matchPrompt1[1];
    }
    else if(matchPrompt2){
         promptValue = matchPrompt2[1];
    }else{
         promptValue = matchPrompt3[1];
    }
    ...
    const prompt = `{{${promptValue}}}, shot 35 mm, realism, octane render, 8k, trending on artstation, 35 mm camera, unreal engine, hyper detailed, photo - realistic maximum detail, volumetric light, realistic matte painting, hyper photorealistic, trending on artstation, ultra - detailed, realistic`
